...
%	\end{table}
	...
	The \acrshort{scs} interfaces with the HP141T subsystem and HP141T Emulator subsystem through the \acrshort{bnc} coaxial cables and prepares the horizontal, vertical and pen-lift outputs to a suitable input voltage range for the unipolar operation STM32H723ZG board, i.e. $\SI{0}{\volt}$ to $\SI{3.3}{\volt}$ DC. Thus, the outputs of the HP8552B \acrshort{if} section or the outputs of the HP141T emulator are the inputs to the \acrshort{scs} as illustrated in figure \ref{fig:sig-cond-bnc-ct-schematic} showing the circuit schematic of the \acrshort{bnc} connector which takes in the vertical output $\texttt{V}_\texttt{y}$.
	
	\begin{figure}[h!]
		\centering
		\includegraphics[width=0.35\linewidth]{Figures/Methodology/sig-cond-bnc-ct-schematic}
		\caption{Schematic of the \acrshort{bnc} connector for receiving the vertical output signal $\texttt{V}_\texttt{y}$ ($\text{V}_\text{vin}$) from the HP8552B plug-in or HP141T Emulator subsystem.}
	...
	\begin{table}[ht!]
		...
		\end{tabular}
	\end{table}
	The \acrshort{scs} is relies on the LM358 op-amp, as shown in the circuit schematics in \ref{fig:signal-conditioning-a} to \ref{fig:signal-conditioning-c}, to scale small voltages and inverting negative voltages. 
