...
def show_help():
    ...
    **Notes:**
    - Data is loaded from 'picoscope_data_X.csv' files in /home/pi.
...
def load_latest_csv():
    try:
        files = [f for f in os.listdir('/home/pi') if f.startswith('picoscope_data_') and f.endswith('.csv')]
        if not files:
    ...
    if not pause_flag:
        ...
        if df is not None:
            try:
                ...
                voltage = df['Channel Voltage (V)'].values
...
try:
    ...
    ttk.Button(root, text="Help", command=show_help).pack()
