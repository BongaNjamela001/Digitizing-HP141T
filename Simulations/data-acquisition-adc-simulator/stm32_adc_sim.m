...
B = 0.4;                % DC offset (V, level-shifted to 0.8V to 1.6V)
...
Vy = 0.001*randn(size(Vy)); % Add Gaussian noise (1mV RMS)

% Characterize the STM32H723ZG ADC
res = 16;               % ADC resolution (16-bit)
Vref = 3.3;             % ADC reference voltage (V, unipolar: 0 to Vref)
Fs_adc = 7000000;       % ADC sampling frequency (Hz, double-interleaved mode)
...
% Simulate double-interleaved mode
...
% Encode quantized signal to binary (16-bit unsigned)
...
% Plot analog, sampled, quantized, and interleaved signals
figure;
% Analog signal
%subplot(4,1,1);
%plot(t, Vy, 'k', 'LineWidth', 1.5);
%title('Analog Vertical Output Signal from the Signal Conditioning Subsystem');
